...
int default_isr_handler(void) {
	writes("Will it be number one?\n", 0, 15);
	__asm__ __volatile__ ("pusha\n\t");
	writes("We are number one", 0, 15);
...
int default_irq_handler(void) {
	...
	uint16_t p = 0x20;
	uint8_t v = 0x20;
	__asm__ __volatile__ ("outb %0, %1" : : "a"(v), "Nd"(p) ); // Send a EOI
	__asm__ __volatile__ ("popa\n\t");
	__asm__ __volatile__ ("iret\n\t");
	return 0;
}
...
void idt_start(void) {
	...
	for (size_t i = 0; i < 32; i++) {
		idt_set_gate(i, (unsigned)default_isr_handler, 0xE, 0x0);
	}
	for (size_t i = 32; i < 48; i++) {
		idt_set_gate(i, (unsigned)default_irq_handler, 0xE, 0x0);
	}
